...
class EQ_Press_Console {
    ...
    static function init() {
        if ( current_user_can( 'manage_options' ) )  {
            if ( ( is_multisite() && is_super_admin() ) || !is_multisite() ) {
                ...
                require_once ( dirname( __FILE__ ) . '/inc/eqpress-console-text.php' );
            }
        }
    }
	...
	static function add_sidebar_menu() {
		...
		add_menu_page( 'eqpress Console', 'eqpress', 'manage_options', 'ep_dashboard', array( __CLASS__, 'eqpress_console_mainpage' ), plugins_url( 'img/eqpress-bolt.png', __FILE__ ), 1 );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Overview', 'Overview', 'manage_options', 'ep_dashboard' );

		add_submenu_page( 'ep_dashboard', 'eqpress Console - Website Stats', 'Website Stats', 'manage_options', 'ep_webstats', array( __CLASS__, 'eqpress_console_webstats' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Manage Cache', 'Manage Cache', 'manage_options', 'ep_cache', array( __CLASS__, 'eqpress_console_cache' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Logs', 'View Logs', 'manage_options', 'ep_logs', array( __CLASS__, 'eqpress_console_logs' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Permissions', 'File Permissions', 'manage_options', 'ep_perms', array( __CLASS__, 'eqpress_console_perms' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Lockdown', 'Security Lockdown', 'manage_options', 'ep_lockdown', array( __CLASS__, 'eqpress_console_lockdown' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Reset Password', 'Reset Password', 'manage_options', 'ep_sftp', array( __CLASS__, 'eqpress_console_sftp_stuff' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Administration Over SSL', 'Admin Over SSL', 'manage_options', 'ep_ssl', array( __CLASS__, 'eqpress_console_wpadmin_ssl' ) );
		
		add_submenu_page( 'ep_dashboard', 'eqpress Console - Plugin and Theme Editor', 'Code Editor', 'manage_options', 'ep_editor', array( __CLASS__, 'eqpress_console_editor' ) );
	...
	/**
	 * eqpress Console dashboard page
	 *
	 */
	static function eqpress_console_mainpage( ) {
		//global $hook_suffix;
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		echo '<div class="wrap">'; //cmohr
		echo '<div id="easy-edit-icon" class="icon32"><img src="' . self::eqpress_console_logo_url() . '">';
		echo '<br /></div>';
		echo '<h2>eqpress Console</h2>';
		echo '<h3>This page provides an overview of the eqpress Console.</h3>';
	 ...
	 *
	 */
	static function eqpress_console_webstats() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
// FIXME - this google url no longer exists
		$stat_warning = <<<'EOW'
		<p id="stat-warning">The following stats will differ significantly from what you see on Google Analytics. This is normal. <!-- <a href="https://support.google.com/analytics/answer/1009616?hl=en" target="_blank">Click here to understand why.</a> -->Google Analytics gathers its statistics from the browser side with embedded code in the web page itself while these stats are gathered directly from the webserver logs.</p>
EOW;
        ...
        echo '<p><a class="button" href="' . admin_url() . 'admin.php?page=ep_webstats&disk_usage=yes">Show Disk Space Utilization</a></p>';
        ...
        if ( isset($_GET['disk_usage']) && $_GET['disk_usage'] == 'yes' ) {
            $pret = self::surf( 'domain=' . $domain . '&do=disk_usage&api_key=' . EP_API_KEY );
            echo $stat_warning;
            echo $pret['body'];
        }
		echo '</div>';
	 ...
	 *
	 */
	static function eqpress_console_cache() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		echo '<div class="wrap">'; //cmohr
	 ...
	 *
	 */
	static function eqpress_console_logs() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
	 ...
	 *
	 */
	static function eqpress_console_perms() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
	 ...
	 *
	 */
	static function eqpress_console_lockdown() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
...
// FIXME -- again fileowner 33, bad
	 ...
	 *
	 */
	static function eqpress_console_sftp_stuff() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
	 ...
	 *
	 */
	static function eqpress_console_wpadmin_ssl() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
	 ...
	 *
	 */
	static function eqpress_console_editor() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
		}
		$domain =  $_SERVER['SERVER_NAME'];
	 ...
	 *
	 */
	static function eqpress_console_logo_url() {
		return plugins_url('img/eqpress-logo.png', __FILE__);
	}
	...
	static function surf( $post_params ) {
		try {
			...
			$params = array( 'url' => 'http://localhost/proxy/eqpress-console-proxy.php',
			...
			error_log( "eqpress-console::surf(): " . $e->getMessage() );
		}
...
add_action( 'plugins_loaded', array( 'EQ_Press_Console',	'init' ) );
